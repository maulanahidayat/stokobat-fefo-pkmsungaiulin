...
$where = "WHERE t.tgl_keluar BETWEEN '$tgl_mulai' AND '$tgl_selesai'";
if ($search != '') {
    $where .= " AND (t.no_transaksi LIKE '%$search%' OR o.nama_obat LIKE '%$search%' OR t.tujuan LIKE '%$search%' OR t.tujuan_pengeluaran LIKE '%$search%')";
}
...
$query = "SELECT t.*, d.jumlah, b.no_batch, b.tgl_kadaluarsa, o.nama_obat, o.kode_obat 
          FROM transaksi_keluar t 
          JOIN detail_keluar d ON t.id = d.transaksi_keluar_id 
          JOIN batch_obat b ON d.batch_id = b.id
          JOIN obat o ON d.obat_id = o.id 
...
// Hitung Total Keluar Periode Ini (Kuantitas)
...
// Hitung Breakdown Kategori Tujuan untuk Periode Ini
$q_breakdown = mysqli_query($conn, "
    SELECT t.tujuan_pengeluaran, COALESCE(SUM(d.jumlah), 0) as total
    FROM detail_keluar d
    JOIN transaksi_keluar t ON d.transaksi_keluar_id = t.id
    JOIN batch_obat b ON d.batch_id = b.id
    JOIN obat o ON d.obat_id = o.id
    $where
    GROUP BY t.tujuan_pengeluaran
");
$breakdown = ['Pasien' => 0, 'Karyawan' => 0, 'Obat Rusak' => 0];
while ($b_row = mysqli_fetch_assoc($q_breakdown)) {
    $breakdown[$b_row['tujuan_pengeluaran']] = $b_row['total'];
}

// Hitung Total Keluar Keseluruhan (Kuantitas)
$q_total_keseluruhan = mysqli_query($conn, "SELECT COALESCE(SUM(jumlah), 0) as total FROM detail_keluar");
...
<div class="text-center mb-4 d-none d-print-block">
    <h3 class="fw-bold mb-1">LAPORAN TRANSAKSI KELUAR OBAT</h3>
...
<!-- Widgets Summary Keluar & Breakdown -->
<div class="row mb-4">
    <div class="col-md-3 mb-2">
        <div class="card shadow-sm border-start border-primary border-4 h-100">
            <div class="card-body py-3">
                <h6 class="text-muted text-uppercase fs-7 mb-1">Total Keluar (Periode)</h6>
                <h4 class="text-primary fw-bold mb-0"><?= number_format($total_periode); ?> <span class="fs-6 fw-normal text-muted">Unit</span></h4>
                <small class="text-muted">Dari total <?= number_format($total_keseluruhan); ?> keseluruhan</small>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-2">
        <div class="card shadow-sm border-start border-info border-4 h-100">
            <div class="card-body py-3">
                <h6 class="text-muted text-uppercase fs-7 mb-1">Pengeluaran Pasien</h6>
                <h4 class="text-info fw-bold mb-0"><?= number_format($breakdown['Pasien']); ?> <span class="fs-6 fw-normal text-muted">Unit</span></h4>
                <small class="text-muted">Distribusi resep obat pasien</small>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-2">
        <div class="card shadow-sm border-start border-success border-4 h-100">
            <div class="card-body py-3">
                <h6 class="text-muted text-uppercase fs-7 mb-1">Pengeluaran Karyawan</h6>
                <h4 class="text-success fw-bold mb-0"><?= number_format($breakdown['Karyawan']); ?> <span class="fs-6 fw-normal text-muted">Unit</span></h4>
                <small class="text-muted">Kebutuhan internal & medis</small>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-2">
        <div class="card shadow-sm border-start border-danger border-4 h-100">
            <div class="card-body py-3">
                <h6 class="text-muted text-uppercase fs-7 mb-1">Obat Rusak (Didepresiasi)</h6>
                <h4 class="text-danger fw-bold mb-0"><?= number_format($breakdown['Obat Rusak']); ?> <span class="fs-6 fw-normal text-muted">Unit</span></h4>
                <small class="text-muted">Akibat kerusakan fisik / wadah</small>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="Cari obat, no transaksi, tujuan, atau tipe..." value="<?= htmlspecialchars($search) ?>">
            </div>
            <div class="col-md-2">
        ...
        <table class="table table-bordered table-striped align-middle text-center">
            <thead class="table-dark">
                <tr>
                    ...
                    <th class="text-start">Tujuan / Detail</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-start"><?= $row['kode_obat'] . ' - ' . htmlspecialchars($row['nama_obat']); ?></td>
                    <td><span class="badge bg-light text-dark border"><?= htmlspecialchars($row['no_batch']); ?></span></td>
